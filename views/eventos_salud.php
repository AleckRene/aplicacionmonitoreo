...
foreach ($records as $record) {
    foreach (['nombre_evento', 'descripcion', 'acciones'] as $key) {
        $valor = isset($record[$key]) ? $record[$key] : null;
        if ($valor !== null) {
...
foreach ($consolidado as &$categoria) {
    foreach ($categoria as $opcion => $cantidad) {
        $categoria[$opcion] = [
            'cantidad' => $cantidad,
